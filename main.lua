...
require "quotes"
require "body" -- include body types
...
char1 = obj {
    nam = "персонаж 1",
    dsc = "Персонаж № 1, {самец}",
    body = body_male,
};

char2 = obj {
    nam = "персонаж 2",
    dsc = "Персонаж № 2, {самочка}",
    body = body_female,
};
...
examine = obj {
    nam = 'Изучить',
    inv = function()
--	p (body_default.waist.nam);
--	p (objs(body_default));
--	for k,v in pairs(body_default) do
--	for k,v in pairs(desk) do
--	    p (k,' == ',v);
--	    p "^";
--	end
--	for k,v in pairs(body_default.bodypart.head) do
--	    p (k,' == ',v);
--	    p "^";
--	end
	p ("Части тела:")
--	p ("^",char1.body.head.nam)
	p ("^ type - ",type(char1.body))
    end,
    use = function(s, w)
	for k,v in pairs(w.body) do
	    p (k,' == ',v);
	    p "^";
	end
    end,
